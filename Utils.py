...
def get_img_seg(path_img, path_softmax, path_seg, height, width, num_classes, resize):
    """
        # Arguments
            path_img: path simple image RGB
            path_softmax: path single softmax numpy
            path_seg: path single ground truth gray scale
            height, width:
            num_classes:
            resize: true or false (must be always true)
        # Returns
            img, softmax, ground truth
...
def data_loader(dir_img, dir_seg, dir_softmax, batch_size, h, w, num_classes, resize):
    """
    # Arguments
        dir_img: path images folder .
        dir_seg: path ground truth images folder, gray scale .
        dir_softmax: path softmax numpy folder
        batch_size:
        h, w : resize value
        num_classes:
        resize: true or false, decide if resize input images
    # Returns
        batch: images, softmax, ground truth
    """
...
def mapCl2Prt():
    """
    :return: a map from 21 classes to 108 parts
    """
    mapPart2Classes = [
        [0, 1],
        [1, 6],
...
def dictImages():
    """
    :return: some image name to be saved during validation callback on last epoch
    """
    img_dict = {
...
def createDirectories(prefix, lr_p, batch_sz, h_img, mult_rate, dil_rate, use_BN):
    """
        Define main and sub directories for the training phase

    :param prefix:
    :param lr_p: learning rate parameter
    :param batch_sz: batch size
    :param h_img: images size
    :param mult_rate: true or false, if the number of channels are multiply by a x factor
    :param dil_rate: true or false, if we are using dilation layer model for softmax input
    :param use_BN: true or false, if we are using batch normalization layer for softmax input
    :return: main path, path for tensorboard, path for checkpoints
    """
...
def list_mult_lr(factor):
    """
        :param factor: multiplier factor for learning rate of these layers
    :return:
    """
...
def listLayer2notFreeze():
    layers = ['conv1_simple',
              ...
              'conv4_BN_simple',
              'conv4_relu_simple',
              'fc1_voc12_c0_combine',
              'fc1_voc12_c1_combine',
              'fc1_voc12_c2_combine',
              'fc1_voc12_c3_combine']
...
def create_excel_file(fileName="results", results21=None, results108=None, path=""):
    """
    :param fileName:
    :param results21: array with IoU for the 21 classes
    :param results108: array with IoU for the 108 parts
...
def adj_func_prova(y_true):
    ...
    for i in range(len(classes)):
        ...
        value = classes[i]
        mask = cv2.inRange(y_true, int(value), int(value))
...
def adj_func_prova_3(y_true):
    adj_mat = np.zeros(shape=(108, 108))
    for o in range(2):

        adj_mat_tmp = np.zeros(shape=(108, 108))
        img = y_true[o]
        classes = np.unique(img)
        classes = classes[1:]
        if 255 in classes:
        ...
        for i in range(len(classes)):
            ...
            for j in range(i + 1, len(classes)):
                ...
                if adj_pixel > 0 and o == 0:
                    adj_mat[classes[i]][classes[j]] = adj_pixel + adj_mat[classes[i]][classes[j]]
                elif adj_pixel > 0:
                    adj_mat_tmp[classes[i]][classes[j]] = adj_pixel + adj_mat_tmp[classes[i]][classes[j]]
        if o > 0:
            adj_mat = np.stack((adj_mat, adj_mat_tmp))

    return adj_mat.astype(np.float32)

# adj_mat = k.zeros(shape=(108, 108))
# for img in range(batch_imgs):
#
#     classes = np.unique(img)
#     classes = classes[1:]
#     if 255 in classes:
#         classes = classes[:-1]
#     mat_contour = k.variable(value=[])
#
#     for i in range(len(classes)):
#
#         value = k.variable(value=classes[i])
#         mask = cv2.inRange(img, int(value), int(value))
#         per, _ = cv2.findContours(image=mask, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_SIMPLE)
#
#         # unisco tutti i perimetri di una parte presente in classes in un unica matrice
#         # matrice di due colonne contenente x e y coordinate dei punti dei perimetri
#
#         mat_total = k.zeros(shape=(1, 2))
#         for q in range(len(per)):
#
...
#             for j in range(len(tmp)):
#                 point = tmp[j]
#                 x = k.variable(point[0][0])
#                 y = k.variable(point[0][1])
#                 mat[j][0] = x
#                 mat[j][1] = y
#
#             mat_total = k.concatenate((mat_total, mat), axis=0)
#
#         # tolto il primo punto 0,0 aggiunto inizialmente
#         mat_contour.append(mat_total[1:])
#
#     for i in range(len(classes)):
#         tmp = mat_contour[i]
