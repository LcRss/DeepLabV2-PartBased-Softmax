...
parser.add_argument("--baseOnEpochs", type=bool, default=False)
...
pathLoadWeights = "Y:/tesisti/rossi/DeeplabV2_resnet101_part/prova.h5"
...
prefix = "Test_incremental_softmax_lr_multiplier_10_softmax_" + loss_name + "_kern_init_" + kernel_init + "_epochs_" + str(epochs_sz) + "_"
...
# train_segs_path = "Y:/tesisti/rossi/data/segmentation_part_gray/dataset_107/data_107part_train/"
...
multipliers = list_mult_lr(10)
opt = LearningRateMultiplier(optimizers.SGD, lr_multipliers=multipliers, lr=lr_p, momentum=0.9)

deeplab_model.compile(optimizer=opt,
                      loss=loss,
...
cb_mIou = CallbackmIoU(path, lr=lr_p, pathRGB=val_images_path, pathSEG=val_segs_path, pathSoft=val_softmax_path,
                       lr_base_on_epochs=baseOnEpochs, max_iter=maxIter)
